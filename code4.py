...
df = df.map(lambda x: color_mapping.get(x, -1))  # Convert color labels to integers
...
"""# Iterate over each row to update node features
for t in range(len(df) - 1):  # Iterate over timesteps
    for i, edge_color in enumerate(df.iloc[t].values):  # Iterate over edges in the current timestep
        node_features[i][edge_color] += 1  # Increment the color feature for the connected nodes"""
