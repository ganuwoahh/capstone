...
def load_configuration(filename='nodes_edges_8.json'):
...
def store_images_data(image_path, weather_data, edge_data):
    ...
    file_exists = os.path.exists('data3.xlsx')

    if file_exists:
        with pd.ExcelWriter('data3.xlsx', engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
            last_row = openpyxl.load_workbook('data3.xlsx')['Sheet1'].max_row
    ...
    print(f"Data for {filename} saved to 'data3.xlsx'.")

# Function to take a screenshot and process it
def take_screenshot_and_process(url, interval):
    screenshots_dir = os.path.join(os.getcwd(), "screenshots/final3")
    ...
    try:
        ...
        while True:  # Infinite loop to take screenshots until interrupted
            ...
            # Load the image and make a copy for analysis
            image = cv2.imread(save_path)
            analysis_image = image.copy()  # Copy for color analysis without highlights

            # Extract edge colors from the original image
            edge_data = {}
            for edge in edges:
                pt1 = tuple(nodes[edge[0]])
                pt2 = tuple(nodes[edge[1]])
            ...
            # Highlight nodes and edges for visualization (using the modified image)
