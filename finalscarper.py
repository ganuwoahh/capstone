...
def store_images_data(image_path, weather_data, edge_data):
    ...
    file_exists = os.path.exists('testdata2.xlsx')

    if file_exists:
        with pd.ExcelWriter('testdata2.xlsx', engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
            last_row = openpyxl.load_workbook('testdata2.xlsx')['Sheet1'].max_row
            df_row.to_excel(writer, index=False, header=False, startrow=last_row)
    else:
        with pd.ExcelWriter('testdata2.xlsx', engine='openpyxl') as writer:
            df_row.to_excel(writer, index=False)

    print(f"Data for {filename} saved to 'testdata2.xlsx'.")

# Function to take a screenshot and process it
def take_screenshot_and_process(url, interval):
    screenshots_dir = os.path.join(os.getcwd(), "screenshots/test8")
    ...
    try:
        ...
        while True:  # Infinite loop to take screenshots until interrupted
            ...
            modified_save_path = os.path.join(screenshots_dir, f"highlighted_{filename}")   #check if edges match
            ...
            for edge in edges:
                pt1 = tuple(nodes[edge[0]])
                pt2 = tuple(nodes[edge[1]])
                avg_color = extract_edge_color(image, pt1, pt2)
