...
def select_edges(image, nodes):
    ...
    print(f"\nYou have selected {len(nodes)} nodes. Specify edges (Right-click to undo).")

    def draw_edges(temp_image, edges, nodes):
        """Redraw the edges on the image."""
    ...
    while True:
        temp_image = image.copy()
        draw_nodes(temp_image, nodes)
        draw_edges(temp_image, edges, nodes)
        cv2.imshow("Image", temp_image)
        key = cv2.waitKey(0)

        if key == ord('d'):  # 'd' to finish
            break
        elif key == ord('u') and edges:  # 'u' to undo the last edge
            edges.pop()
        else:
            try:
                edge_input = input("Enter an edge as 'start_node end_node' (e.g., '0 1'): ")
                start, end = map(int, edge_input.split())
                if 0 <= start < len(nodes) and 0 <= end < len(nodes) and start != end:
                    edges.append((start, end))
                else:
                    print(f"Invalid edge. Ensure indices are between 0 and {len(nodes)-1} and not the same.")
            except ValueError:
...
def save_configuration(nodes, edges, filename='nodes_edges.json'):
    data = {'nodes': nodes, 'edges': edges}
    with open(filename, 'w') as f:
        json.dump(data, f)

# 4. Load the configuration
def load_configuration(filename='nodes_edges.json'):
...
if __name__ == '__main__':
    ...
    image_path = r'D:\code_stuff\cap\test_1.jpeg'
